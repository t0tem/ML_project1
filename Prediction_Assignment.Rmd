...
After taking a look at data (output omitted as it's too long)
```{r results='hide'}
str(sub.training)
```
we see that many variables contain a lot of missing values.  
So next move is to remove from training set all variables with NAs
